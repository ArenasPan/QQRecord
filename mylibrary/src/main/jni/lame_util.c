...
Java_com_example_mylibrary_utils_LameUtil_init(JNIEnv *env, jclass type, jint inSamplerate,
...
Java_com_example_mylibrary_utils_LameUtil_encode(JNIEnv *env, jclass type,
...
JNIEXPORT jint JNICALL
Java_com_example_mylibrary_utils_LameUtil_flush(JNIEnv *env, jclass type, jbyteArray mp3buf) {
    const jsize mp3buf_size = (*env)->GetArrayLength(env, mp3buf);
    jbyte* j_mp3buf = (*env)->GetByteArrayElements(env, mp3buf, NULL);
...
JNIEXPORT void JNICALL
Java_com_example_mylibrary_utils_LameUtil_close(JNIEnv *env, jclass type) {
    lame_close(lame);
    lame = NULL;
}